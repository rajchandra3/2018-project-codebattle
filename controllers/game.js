...
exports.game = function(req, res){ //Check if it is first time and if so set time
  Match.
    ...
    exec(function(err, match){
      ...
      if(!match || match.active == false){
        ...
        res.render('game/match',{task_description: match.taskID.description, matchID: req.body.matchID});
      }
    });
  }
  ...
  exports.post = function(req,res){
    ...
    var output = [];
    var errors = [];
    Match.findOne({_id:req.body.matchID, $or:[{'player1': req.session.user._id}, {'player2': req.session.user._id}]}, function (err, match){
      if(!match){
        errors.push({param: '', msg: 'Something went wrong when submiting', value: ''});
        res.redirect('/', {
          errors: errors
        });
      }
